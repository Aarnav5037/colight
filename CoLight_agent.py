...
class CoLightAgent(Agent): 
    ...
    def MultiHeadsAttModel(self,In_agent,In_neighbor,l=5, d=128, dv=16, dout=128, nv = 8,suffix=-1):
        ...
        neighbor_repr = Lambda(
            lambda x, tf=tf: tf.matmul(x[0], x[1]),
        ...
        att = Lambda(lambda x, tf=tf: K.softmax(tf.matmul(x[0], K.permute_dimensions(x[1], (0, 1, 2, 4, 3)))),
        ...
        out = Lambda(lambda x, tf=tf: tf.matmul(x[0], x[1]),
